 ...
 *   string $printer_url     – Printer HTTP URL from Settings (may be empty)
 *   bool   $autoprint       – Auto-send to the printer / open print dialog
 *
...
<body>
	...
	<div id="ctci-print-status" class="print-status" role="status" aria-live="polite"></div>

	<script>
		(function () {
			'use strict';

			var printerUrl = <?php echo wp_json_encode($printer_url); ?>;
			var autoprint = <?php echo $autoprint ? 'true' : 'false'; ?>;
			var statusEl = document.getElementById('ctci-print-status');

			function setStatus(msg, isError) {
				if (!statusEl) return;
				statusEl.textContent = msg;
				statusEl.className = 'print-status' + (isError ? ' is-error' : '');
			}

			function joinQuery(base, qs) {
				if (base.indexOf('?') === -1) return base + '?' + qs;
				if (base.endsWith('?') || base.endsWith('&')) return base + qs;
				return base + '&' + qs;
			}

			// Send the badge data (this page's own query params) to the printer.
			// Resolves on any HTTP response; rejects on network failure/timeout.
			function sendToPrinter() {
				...
				fetch(joinQuery(printerUrl, q.toString()), {
					method: 'GET',
					mode: 'no-cors',
					cache: 'no-store',
					...
					.then(function () {
						clearTimeout(timer);
						setStatus('\u2713 Badge sent to printer');
					})
					.catch(function () {
						clearTimeout(timer);
						setStatus('Printer URL unreachable \u2014 printing from this device.', true);
						window.print();
					});
			}

			// Fallback path: a printer URL is configured but unreachable, so the
			// badge page auto-opens the browser print dialog to use a printer
		...
		<div class="badge-body">
				...
				<ul class="badge-socials">
					...
					<?php if ($twitter): ?>
						<li>
							<svg viewBox="0 0 20 20" aria-hidden="true">
								<path
									d="M11.9 8.63 18.6 1h-1.6l-5.8 6.62L6.55 1H1l7.03 9.9L1 19h1.6l6.13-6.98L13.65 19H19l-7.1-10.37Zm-2.17 2.47-.71-.99L3.4 2.15h2.44l4.55 6.37.71.99 5.92 8.28h-2.44l-4.85-6.7Z" />
							</svg>
					...
					<?php if ($website_display): ?>
						<li>
							<svg viewBox="0 0 20 20" aria-hidden="true">
								<path
									d="M10 1a9 9 0 1 0 0 18 9 9 0 0 0 0-18Zm6.93 8.25h-3.05a13.4 13.4 0 0 0-1.14-5.4 7.52 7.52 0 0 1 4.19 5.4Zm-6.93-6.5c.7 0 1.8 1.94 2.12 5h-4.24c.32-3.06 1.42-5 2.12-5ZM7.7 3.85A13.4 13.4 0 0 0 6.56 9.25H3.51a7.52 7.52 0 0 1 4.2-5.4Zm-4.2 6.9h3.05c.1 1.98.5 3.83 1.14 5.4a7.52 7.52 0 0 1-4.2-5.4Zm4.42 0h4.24c-.32 3.06-1.42 5-2.12 5-.7 0-1.8-1.94-2.12-5Zm4.5 5.4c.63-1.57 1.03-3.42 1.13-5.4h3.05a7.52 7.52 0 0 1-4.19 5.4Z" />
							</svg>
			...
			<?php if ($meal_pref): ?>
				<div class="badge-meal">
					<svg viewBox="0 0 20 20" aria-hidden="true">
						<path
							d="M4 1v7a2 2 0 0 0 1.5 1.94V19h1V9.94A2 2 0 0 0 8 8V1H7v6H6V1H5v6H4.5V1H4Zm10.5 0a2.5 2.5 0 0 0-2.5 2.5V9a1 1 0 0 0 1 1h.5v9h1v-9H15V1h-.5Z" />
					</svg>
